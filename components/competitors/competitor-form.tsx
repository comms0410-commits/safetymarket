...
export function CompetitorForm({ competitor, action, compact = false }: { competitor?: Competitor; action: (formData: FormData) => Promise<void>; compact?: boolean }) {
  const field = "rounded-lg border border-border px-3 py-2 text-sm";
  return (
    <form action={action} className={compact ? "grid gap-4" : "grid gap-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"}>
      ...
      <div className="flex justify-end gap-2"><Button href="/competitors" variant="secondary">취소</Button><Button>{competitor ? "수정 저장" : "경쟁사 등록"}</Button></div>
    </form>
  );
}
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  ...
  return <label className="grid gap-1 text-sm font-medium text-slate-700">{label}<input {...inputProps} className="rounded-lg border border-border px-3 py-2 text-sm" /></label>;
}

function Textarea({ label, ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }) {
  return <label className="grid gap-1 text-sm font-medium text-slate-700">{label}<textarea {...props} rows={3} className="rounded-lg border border-border px-3 py-2 text-sm" /></label>;
}
