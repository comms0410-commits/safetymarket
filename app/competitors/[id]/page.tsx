...
import { collectedItems, findCompetitor, insights, monitoredSources } from "@/lib/sample-data";
...
export function generateStaticParams() {
  return ["jt", "sh", "kichang", "weltizen", "ealim", "ntrack", "safeadvisor"].map((id) => ({ id }));
}
...
export default async function CompetitorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const competitorInsights = insights.filter((insight) => insight.competitorId === competitor.id).slice(0, 5);
  const competitorItems = collectedItems.filter((item) => item.competitorId === competitor.id).slice(0, 5);
  const competitorSources = monitoredSources.filter((source) => source.competitorId === competitor.id);
      ...
      <div className="space-y-6">
        <section className="saas-card overflow-hidden">
          <div className="bg-slate-950 p-5 text-white sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <form action={deleteCompetitor}><Button variant="danger">삭제 데모</Button></form>
              </div>
            </div>
          </div>
          <div className="grid gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4 lg:p-6">
            ...
            <Info label="수집 채널" value={`${competitorSources.length}개`} />
            <Info label="최근 이슈" value={`${competitorInsights.length}개`} />
          </div>
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-[1fr_420px]">
          ...
          <div className="saas-card p-5 sm:p-6">
            ...
            <p className="mt-1 text-sm text-slate-500">초기 배포 안정형 버전에서는 저장 없이 화면 흐름만 제공합니다.</p>
